 ...
 * 加载首页的文章
 */
@WebServlet("/history/xxgg")
public class ShowXxggServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int pre = pageindex - 1;
        int total = new PageDAO().XxggTotal();
        int last;
...
//		List<News> allList = NewsService.shwoAll(allNews);
		...
		request.setAttribute("xxggList", newList);
//		request.setAttribute("allList", allList);
//		System.out.println(newList.toString()); 
		//response.sendRedirect(request.getContextPath() + "/news/index.jsp");
		request.getRequestDispatcher("xxgg.jsp").forward(request, response);
	}
}
