 ...
 * 加载首页的文章
 */
@WebServlet("/history/shgc")
public class ShowShgcServlet extends HttpServlet {
...
//		News allNews = new News();
        ...
        int pre = pageindex - 1;
        int total = new PageDAO().ShgcTotal();
        int last;
...
//		List<News> allList = NewsService.shwoAll(allNews);
		...
		request.setAttribute("shgcList", newList);
//		request.setAttribute("allList", allList);
//		System.out.println(newList.toString()); 
		//response.sendRedirect(request.getContextPath() + "/news/index.jsp");
		request.getRequestDispatcher("shgc.jsp").forward(request, response);
	}
}
